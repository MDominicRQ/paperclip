...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    void heartbeat
      ...
      .then(async () => {
        const drift = await heartbeat.reconcileHermesAdapterDrift();
        if (drift.driftDetected > 0 || drift.driftReconciled > 0) {
          logger.warn({ ...drift }, "startup hermes drift reconciliation updated runtime state");
        }
      })
      .catch((err) => {
    ...
    setInterval(() => {
      ...
      void heartbeat
        ...
        .then(async () => {
          const drift = await heartbeat.reconcileHermesAdapterDrift();
          if (drift.driftDetected > 0 || drift.driftReconciled > 0) {
            logger.warn({ ...drift }, "periodic hermes drift reconciliation updated runtime state");
          }
        })
        .catch((err) => {
